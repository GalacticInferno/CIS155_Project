	...
	public void update(int key)
	{
		...
		case 84:
			if (y > yTop ) {
				...
				{
					if (GameLoop.Bil_O.get(GameLoop.Bil_O.size()-1).getY() > yTop) {
						y -= moveSpeed;
						for (int i =0; i < GameLoop.Bil_O.size(); i++)
		            	{
		            		GameLoop.Bil_O.get(i).update(key);
		...
		case 72:
			if (x < xRight) {
				...
				{
					if (GameLoop.Bil_O.get(GameLoop.Bil_O.size()-1).getX() < xRight) { // gets the location of random wall and tests if it would go over the border
						x += moveSpeed;
						for (int i =0; i < GameLoop.Bil_O.size(); i++)
		            	{
		            		GameLoop.Bil_O.get(i).update(key);
		...
		case 71:
			if (y<yBottom){
				...
				{
					if (GameLoop.Bil_O.get(GameLoop.Bil_O.size()-1).getY() < yBottom) { // gets the location of random wall and tests if it would go over the border
						y += moveSpeed;
						for (int i =0; i < GameLoop.Bil_O.size(); i++)
		            	{
		            		GameLoop.Bil_O.get(i).update(key);
		...
		case 70:
			if(x>xLeft){
					...
					{
						if (GameLoop.Bil_O.get(GameLoop.Bil_O.size()-1).getX() > xLeft) { // gets the location of random wall and tests if it would go over the border
							x -= moveSpeed;
							for (int i =0; i < GameLoop.Bil_O.size(); i++)
			            	{
			            		GameLoop.Bil_O.get(i).update(key);
